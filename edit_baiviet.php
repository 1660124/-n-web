...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<?php 
			...
			if($_SERVER['REQUEST_METHOD']=='POST')
			{
				...
				if(empty($errors))
				{
					...
					else
					{
						...
						$sql="SELECT anh,anh_thumb FROM tblbaiviet WHERE id={$id}";
			...
			$query_id="SELECT danhmucbaiviet,title,tomtat,noidung,anh,anh_thumb,ngaydang,giodang,ordernum,status FROM tblbaiviet WHERE id={$id}";
			$result_id=mysqli_query($dbc,$query_id);
			kt_query($result_id,$query_id);
			//Kiểm tra xem ID có tồn tại không
			if(mysqli_num_rows($result_id)==1)
			{
				list($danhmucbaiviet,$title,$tomtat,$noidung,$anh,$anh_thumb,$ngaydang,$giodang,$ordernum,$status)=mysqli_fetch_array($result_id,MYSQLI_NUM);
			}
			else
			{
		...
		<form name="frmbaiviet" method="POST" enctype="multipart/form-data">
			...
			<div class="form-group">
				<label>Thứ tự</label>
				<input type="text" name="ordernum" value="<?php if(isset($ordernum)){ echo $ordernum;} ?>" class="form-control">
			</div>
			<div class="form-group">
